...
def get_enemy_direction(player_pos, enemy_pos):
    ...
    ollama_response = ollama.generate(model='llama3', prompt= question + " You are running away from the enemy, in which direction will you run? Answers need to be one word and from this list - 'Left', 'Right', 'Down', 'Up';", context = conversation_context)
    #update_context(ollama_response['context'])
    # Get the response
    answer = ollama_response['response'].strip().lower()
    print(answer)
    return answer
