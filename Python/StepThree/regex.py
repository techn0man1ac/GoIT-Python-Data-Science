...
_text = "Python - це проста, але потужна мова програмування."
_pattern = r"\w+"
_matches = re.findall(_pattern, _text)

print(_matches)  # Виведе список всіх слів у рядку
# ['Python', 'це', 'проста', 'але', 'потужна', 'мова', 'програмування']

text_e = "Контакти: example1@example.com, example2@sample.org"
pattern_e = r"\w+@\w+\.\w+"
matches_e = re.findall(pattern_e, text_e)

print(matches_e)  # Виведе всі знайдені електронні адреси
# ['example1@example.com', 'example2@sample.org']

'''
Метод re.findall корисний, коли вам потрібно 
зібрати всі відповідності регулярного виразу 
в рядку, наприклад, зібрати всі електронні адреси 
з тексту або всі слова, що задовольняють певному шаблону.
Цей метод часто використовується в задачах обробки 
тексту, де необхідно знайти і вилучити інформацію 
з великих обсягів тексту.
'''